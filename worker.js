// Cloudflare Worker：Telegram 双向机器人 v1.0

// --- 配置常量 ---
const CONFIG = {
    VERIFY_ID_LENGTH: 12,
    VERIFY_EXPIRE_SECONDS: 300,         // 5分钟
    VERIFY_MAX_FAILED_ATTEMPTS: 2,      // 答错次数达到上限后重置验证，防止穷举按钮
    VERIFY_FAIL_COOLDOWN_SECONDS: 600,  // 答错达到上限后冷却 10 分钟
...
function generateMathChallenge() {
    const a = secureRandomInt(6, 30);
    const b = secureRandomInt(3, 20);
    const useSubtraction = secureRandomInt(0, 2) === 1;
    const c = useSubtraction ? secureRandomInt(1, Math.min(15, a + b)) : secureRandomInt(2, 12);
    const answer = useSubtraction ? a + b - c : a + b + c;
    ...
    while (options.size < 4) {
        const delta = secureRandomInt(-9, 10);
        if (delta === 0) continue;
        const candidate = answer + delta;
        if (candidate >= 0) options.add(String(candidate));
    }
...
async function sendVerificationChallenge(userId, env, pendingMsgId) {
    ...
    if (cooldown) {
        Logger.info('verification_cooldown_active', { userId, hasPending: !!pendingMsgId });
        await tgCall(env, "sendMessage", {
            chat_id: userId,
            text: "⚠️ 验证失败次数过多，请10分钟后再试。"
        });
        return;
    }
...
async function handleCallbackQuery(query, env, ctx) {
    try {
        ...
        if (selectedIndex === state.answerIndex) {
            ...
            const shouldResetChallenge = failedAttempts >= CONFIG.VERIFY_MAX_FAILED_ATTEMPTS;
            ...
            if (shouldResetChallenge) {
                ...
                if (query.message?.message_id) {
                    try {
                        ...
                        Logger.warn('verification_failure_message_edit_failed', { userId, verifyId });
                    }
                }
                return;
            }

            state.failed_attempts = failedAttempts;
            await env.TOPIC_MAP.put(`chal:${verifyId}`, JSON.stringify(state), { expirationTtl: CONFIG.VERIFY_EXPIRE_SECONDS });
